...
CoAPServer.prototype.listen = function(port, address, done) {
  ...
  this._sock = dgram.createSocket(this._options.type, function(msg, rsinfo) {
    ...
    that._handle(packet, rsinfo, done);
  })

  var errMulticast = false;

  this._sock.bind(port, address || '0.0.0.0', function(){
    // the 0.0.0.0 is to deal with this issue: https://github.com/joyent/node/issues/4944
    if(typeof that._options.addMembership == 'object' && util.isArray(that._options.addMembership)) {
      for(var n=0;n<that._options.addMembership.length;n++) {
        var el = that._options.addMembership[n];
        try {
          that._sock.setBroadcast(true);
          if(typeof el == 'string') {
            that._sock.addMembership(el);
          } else if (typeof el == 'object' && util.isArray(el)) {
            that._sock.addMembership(el[0],el[1]); 
          }
        } catch(e) {
          errMulticast = true;
          done.call(this,e);
    ...
    if(done && !errMulticast) done.apply(this,arguments); // call done like normal
  });
  this._port = port
